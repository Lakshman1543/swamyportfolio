...
const AboutSection = () => {
  const skills = {
    'Languages': ['Java', 'Python', 'SQL', 'PL/SQL', 'C'],
          ...
          <div className="space-y-6">
            <div className="glass-card p-8 neon-glow">
              <p className="text-lg text-gray-300 leading-relaxed mb-6">
                I'm <span className="text-cyan-400 font-semibold">Lakshmana Swamy</span>, a passionate B.Tech Computer Science Engineering student from 
                  ...
                  <div key={category} className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {skillList.map((skill) => (
                        <span
                          key={skill}
                          className="px-3 py-1 text-xs bg-white/10 border border-white/20 rounded-full text-gray-300 hover:text-white hover:border-neon-cyan transition-all duration-300"
                        >
                          {skill}
                        </span>
                      ))}
